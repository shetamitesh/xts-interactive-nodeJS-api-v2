...
module.exports = class XTSInteractive {
  ...
  async getDealerPositions(reqObject) {
    try {
      ...
      let response = await request.processRequest(
        'GET',
        this.url +
        settings.restApi.dealerposition +
        '?dayOrNet=' +
        reqObject.dayOrNet +
        '&clientID=' +
        reqObject.clientID,
   ...
   *
   * @param {string} reqObject.exchangeSegment
   *  exchangeSegment of the particular order
   *
   * @param {string} reqObject.exchangeInstrumentID
   *  exchangeInstrumentID of the particular order.
   */
  async cancelAllOrder(reqObject) {
    try {
      await this.checkLoggedIn();
      let response = await request.processRequest(
        'POST',
        this.url + settings.restApi.cancelall,
        { authorization: this.token },
        reqObject
      );
      return response;
  ...
  /**
   * squareOff API of the application provides functionality of square off position in the application
   *
   * @param {Object} reqObject request object.
   *
   * @param {string} reqObject.exchangeSegment
   *  NSECM, BSECM, NSEFO, NSECD segment of the particular stock
   * 
   * @param {string} reqObject.exchangeInstrumentID
   *  token of the particular stock
   * 
   * @param {string} reqObject.productType
   *  MIS or NRML product type
   * 
   * @param {string} reqObject.squareoffMode
   *  DayWise or NetWise mode
   * 
   * @param {string} reqObject.positionSquareOffQuantityType
   *  ExactQty or Percentage square off order type
   * 
   * @param {string} reqObject.squareOffQtyValue
   *  Either exact qty or percentage value in number
   * 
   * @param {string} reqObject.blockOrderSending
   *  Allow block order sending 
   * 
   * @param {string} reqObject.cancelOrders
  ...
  /**
   * squareOffAll API of the application provides functionality of square off all position in the application
   *
   * @param {Object} reqObject request object.
   * 
   * @param {string} reqObject.squareoffMode
   ...
   *
   */
  async squareOffAll(reqObject) {
    try {
      await this.checkLoggedIn();
      let response = await request.processRequest(
        'PUT',
        this.url + settings.restApi.squareoffall,
        { authorization: this.token },
        reqObject
      );
      return response;

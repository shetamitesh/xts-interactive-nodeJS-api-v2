...
module.exports = class XTSEmitter {
  ...
  init(reqObject) {
    ...
    /**
     * Listener of the pong event via socket and emit the pong event via event Emitter
     *
     *
     * @event pong
     */
    this.socketMD.socketMarketData.on(settings.socket.pong, (data) => {
      console.info('socket pong received');
      this.eventEmitter.emit(settings.socket.pong, data);
    });
   ...
   */
  onPong(fn) {
    this.eventEmitter.on(settings.socket.pong, (data) => {
      fn(data);
    });
  }
